...
void try_update_best(const std::vector<int> &candidate, const TimePoint& start_time, double& best_time, int& best_fitness, std::vector<int>& best_solution);
...
Individual evaluate_chromosome(std::vector<double> chromosome, std::mt19937 &rng, const Params& params, bool is_mutant);
...
Params parse_arguments(int argc, char *argv[]) {
    //Por defecto dejamos los valores optimos encontrados en la entrega anterior
    Params p;
    ...
    if (p.max_seconds <= 0) p.max_seconds = 1;
        p.population_size = std::clamp(p.population_size, 4, 100000); 
        p.elite_fraction = std::clamp(p.elite_fraction, 0.0, 1.0);
        p.mutant_fraction = std::clamp(p.mutant_fraction, 0.0, 1.0);
        p.inheritance_prob = std::clamp(p.inheritance_prob, 0.0, 1.0);
        p.local_search_probability = std::clamp(p.local_search_probability, 0.0, 1.0);
        p.alpha_grasp = std::clamp(p.alpha_grasp, 0.0, 1.0);
